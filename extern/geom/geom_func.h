 ...
 * ------------------------------------------------------------------------------
 * ------------------------------------------------------------------------------*/
#ifndef __dt_geom_func_h__
#define __dt_geom_func_h__
...
namespace dt {
	namespace GEOM_FUNC
	{
		...
		//double orient2dIn3dAbsFast(double* pa, double* pb, double* pc);
		...
		extern int lin_lin_intersect3d_idx_noOverlapPnt(int iline[2][2], double linep[2][2][3], int intTyp[2], int intIdx[2]);

		// 0: 不相交
		// 1: 相交于唯一点，intPnt 有效
		// 2: 共线重叠，交集不是唯一点，intPnt 不建议使用
		extern int lin_lin_intersect3d_exact(double linep[2][2][3],int intTyp[2],int intIdx[2],double intPnt[3]);
